...
public partial class EntityDefinitionBuilder
{
    ...
    public EntityDefinition Build(Type entityType)
    {
        ...
        // فهارس CHECK
        // ✅ استدعاء الميثود الجديدة لفهارس الـ CHECK
    ...
    /// </summary>
    private static bool IsMaxLength(string typeName) =>
    ...
    private static PrimaryKeyDefinition? GetPrimaryKey(Type type)
    {
        ...
        var pkColumns = type
            ...
            .Select(p => new { Name = GetColumnName(p), Description = p.GetCustomAttribute<DescriptionAttribute>()?.Description })
            .ToList();

        if (pkColumns.Count > 0)
        {
            var names = pkColumns.Select(x => x.Name).ToList();
            var descriptions = string.Join("; ", pkColumns.Select(x => x.Description).Where(d => !string.IsNullOrWhiteSpace(d)).ToList());
            Console.WriteLine($"[TRACE:PK] Final PK for {type.Name} (via [Key]): {string.Join(", ", names)}");
            ...
            return new PrimaryKeyDefinition
            {
                ...
                Name = $"PK_{type.Name}",
                Description = !string.IsNullOrWhiteSpace(descriptions) ? descriptions : null
            };
        }
        ...
        if (pkColumns.Count > 0)
        {
            ...
            Console.WriteLine($"[TRACE:PK] Final PK for {type.Name} (via naming): {string.Join(", ", names)}");
        ...
        if (idCols.Count >= 2)
        {
            var names = idCols.Select(x => x.Name).ToList();
            var descriptions = string.Join("; ", idCols.Select(x => x.Description).Where(d => !string.IsNullOrWhiteSpace(d)).ToList());

            Console.WriteLine($"[TRACE:PK] Composite PK inferred for {type.Name}: {string.Join(", ", names)}");
        ...
        if (entity.PrimaryKey?.Columns != null && entity.PrimaryKey.Columns.Any())
        {
            ...
            foreach (var pkName in entity.PrimaryKey.Columns)
            {
                ...
                if (pkCol != null)
                {
                    ...
                    bool isEnum = pkCol.PropertyType != null && pkCol.PropertyType.IsEnum;
                    Type? underlyingType = isEnum ? pkCol.PropertyType.GetEnumUnderlyingType() : null;

                    pkCol.IsIdentity =
                        !isComposite &&
                        entity.PrimaryKey.IsAutoGenerated &&
                        pkCol.PropertyType != null &&
                        pkCol.PropertyType != typeof(string) &&
                        underlyingType != typeof(string) &&
                        pkCol.PropertyType != typeof(Guid) &&
                    ...
                    if (IsTextType(pkCol.TypeName) && IsMaxLength(pkCol.TypeName))
                    {
                        int safeLength = 450;
                        pkCol.TypeName = $"nvarchar({safeLength})";
                        Console.WriteLine($"[AUTO-FIX] Changed {entity.Name}.{pkCol.Name} from nvarchar(max) to nvarchar({safeLength}) for primary key safety.");
                    }
                }
            }
        }
    ...
    internal static void NormalizeConstraints(EntityDefinition entity)
    {
        ...
        if (entity.PrimaryKey != null)
        {
            constraints.Add(new ConstraintDefinition
            {
                ...
                Type = "PRIMARY KEY",
                Columns = entity.PrimaryKey.Columns.ToList(),
                ReferencedColumns = entity.PrimaryKey.Columns.ToList(),
                Description = entity.PrimaryKey.Description
            });
        }
        ...
        foreach (var uq in entity.UniqueConstraints)
        {
            constraints.Add(new ConstraintDefinition
            {
                Name = uq.Name,
                Type = "UNIQUE",
                Columns = uq.Columns.ToList(),
                ReferencedColumns = uq.Columns.ToList(),
        ...
        foreach (var fk in entity.ForeignKeys)
        {
            constraints.Add(new ConstraintDefinition
            {
                Name = string.IsNullOrWhiteSpace(fk.ConstraintName)
                ...
                ReferencedColumns = new List<string> { fk.ReferencedColumn },
        ...
        foreach (var ck in entity.CheckConstraints)
        {
            constraints.Add(new ConstraintDefinition
            {
                Name = ck.Name,
                Type = "CHECK",
                Columns = new List<string>(), // CHECK مش مربوط مباشرة بأعمدة
                ReferencedColumns = ck.ReferencedColumns.ToList(),
                DefaultValue = ck.Expression,
                Description = ck.Description
            });
        }

        // 🟠 Default Constraints (من الأعمدة)
        foreach (var col in entity.Columns.Where(c => !string.IsNullOrWhiteSpace(c.DefaultValue?.ToString())))
        {
            constraints.Add(new ConstraintDefinition
            {
                Name = $"DF_{entity.Name}_{col.Name}",
                Type = "DEFAULT",
                Columns = new List<string> { col.Name },
