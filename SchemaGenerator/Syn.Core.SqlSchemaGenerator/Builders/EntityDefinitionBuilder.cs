...
public partial class EntityDefinitionBuilder
{
    ...
    public void BuildColumn(PropertyInfo prop, EntityDefinition entity)
    {
        ...
        if (IsTextType(columnModel.TypeName) && IsMaxLength(columnModel.TypeName))
        {
            ...
            //else if(columnModel.IsPrimaryKey && columnModel.TypeName == "nvarchar(max)")
            //{
            //    int safeLength = 450;
            //    columnModel.TypeName = $"nvarchar({safeLength})";
            //    Console.WriteLine($"[AUTO-FIX] Changed {entity.Name}.{columnModel.Name} from nvarchar(max) to nvarchar({safeLength}) for primary key safety.");
            //}
        }
        ...
        if (entity.PrimaryKey?.Columns != null && entity.PrimaryKey.Columns.Any())
        {
            ...
            foreach (var pkName in entity.PrimaryKey.Columns)
            {
                ...
                if (pkCol != null)
                {
                    ...
                    pkCol.IsIdentity = !isComposite && entity.PrimaryKey.IsAutoGenerated && pkCol.PropertyType != typeof(string) && pkCol.PropertyType?.GetEnumUnderlyingType() != typeof(string);
